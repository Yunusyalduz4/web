  ...
  const handleSubmit = async (e: any) => {
    ...
    try {
      ...
      const cleanForm = {
        ...
        instagram: form.instagram?.trim() === '' ? undefined : form.instagram?.trim(),
        ...
        {employees?.map((e: any) => (
          <div key={e.id} className="bg-white/70 backdrop-blur-md rounded-2xl shadow-sm p-3 sm:p-4 hover:shadow-md active:shadow-lg transition-all border-2 border-transparent"
            ...
            <div className="flex items-start justify-between gap-2 sm:gap-3 mb-3">
              ...
              {e.instagram && (
                <div className="text-right shrink-0">
                  <div className="text-[10px] sm:text-xs text-gray-600">Instagram</div>
                  <a 
                    href={`https://instagram.com/${e.instagram.replace('@', '')}`} 
                    target="_blank" 
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 text-xs sm:text-sm font-semibold text-pink-600 hover:text-pink-700 transition-colors"
                  >
